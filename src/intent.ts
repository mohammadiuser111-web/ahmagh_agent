...
/**
 * درخواستِ «مصرف/هزینه/سهمیه» — قبل از خروجی چک می‌شود («گزارش هزینه» → مصرف، نه فایل).
 * محافظ در برابر مثبت کاذب: اگر جمله درباره‌ی ساخت/یادآوری/تسک است، مصرف نیست.
 */
export function detectUsageQuery(text: string): boolean {
  const t = text.replace(/احمق/g, " ").replace(/\u200c/g, " ").trim();
  if (/تسک|تاسک|بساز|ایجاد|ساخت|ثبت|یادآوری|یادم|تمیز|بخر|خرید|ویرایش|حذف/.test(t)) return false;
  return /هزینه|مصرف|سهمیه|لیمیت|نورون|قیمت|چقدر\s*جا|چند\s*جا|چقدر\s*فضا|چند\s*فضا|\bcost\b|\busage\b|\blimit\b/i.test(t);
}
