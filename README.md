...
|---|---|---|
| عنوان تسک | `title` | **اجباری** — از متن کاربر استخراج می‌شود |
...
| تاریخ شروع | `start_date` (+ `started_at` واقعی) | YYYY-MM-DD؛ اگر نگویی = **امروز** |
| تاریخ ساخت | `created_at` | **اجباری** — خودکار |
| تاریخ پایان | `due_date` (+ `completed_at` واقعی) | **اجباری** — اگر نگویی، بات جدا می‌پرسد؛ «تا فردا»، «تا پنجشنبه»، «تا ۱۵ مهر» و… را می‌فهمد |
| یادآوری | `last_reminded_at` + `reminder_count` | موتور یادآوری را راه می‌اندازد |

### دو قانون طلایی

- 📅 **تاریخ پایان اجباری است:** اگر در متن نگی، بات جدا می‌پرسد («تا کی باید تموم بشه؟») و جوابت را می‌فهمد — «تا فردا»، «پنجشنبه»، «۱۵ مهر»، «۳ روز دیگه»…
- 🚦 **شروع خودکار:** اگر تاریخ شروع نگی = امروز؛ و با رسیدن تاریخ شروع (تیک بعدی کرون)، وضعیت تسک خودکار «در حال انجام» می‌شود و به مسئول پیام می‌رود.
