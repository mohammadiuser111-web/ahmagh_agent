...
| `/edit <شناسه> <فیلد>: <مقدار>` | ✏️ ویرایش تسک (فاز ۳) |
...
فیلدهای مجاز: `عنوان`، `توضیح`، `مسئول`، `شروع`، `پایان` (یا `سررسید`/`مهلت`)، `وضعیت`:

```text
/edit 12 عنوان: خرید نون
/edit 12 پایان: فردا ساعت ۱۰:۳۰ عصر
/edit 12 شروع: شنبه ساعت ۸ صبح
/edit 12 مسئول: علی
/edit 12 وضعیت: در حال انجام
```

### 🕐 ساعت دقیق (فاز ۳)

علاوه بر تاریخ، ساعت هم می‌فهمیم: «تا فردا ساعت ۱۰:۳۰ عصر»، «۱۲ ظهر»، «۸ صبح»، «۵ عصر»، «۱۰ شب».
زمان‌ها با آفست تهران (+03:30) ذخیره می‌شوند و یادآوری‌ها/شروع خودکار لحظه‌ی دقیق را می‌سنجند.

### 🚦 شروع خودکار دقیق

اگر زمان شروع در آینده باشد (تاریخ یا ساعت)، تسک با پرچم `auto_start` ذخیره می‌شود و کرون هر ۱۰ دقیقه، در لحظه‌ی رسیده، وضعیت را به «در حال انجام» تغییر می‌دهد و به مسئول پیام می‌دهد. تسک‌های «از امروز» عمداً این پرچم را نمی‌گیرند تا اسپم نشوند.
